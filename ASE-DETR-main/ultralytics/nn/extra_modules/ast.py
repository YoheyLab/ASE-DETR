...
__all__ = ['AdaptiveSparseSA', 'FRFN2D']
...
class WindowAttention_sparse(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        if self.ABLATION_MODE == 'dsa_only':
            final_attn = self.softmax(attn)
            
        elif self.ABLATION_MODE == 'ssa_only':
            final_attn = self.relu(attn)**2
        ...
        x = (final_attn @ v).transpose(1, 2).reshape(B_, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x

    def extra_repr(self) -> str:
        return f'dim={self.dim}, win_size={self.win_size}, num_heads={self.num_heads}, mode={self.ABLATION_MODE}'
...
class AdaptiveSparseSA(nn.Module):
    ...
    def forward(self, x, mask=None):
        # B, L, C = x.shape
        B, C, H, W = x.size()
        x = x.flatten(2).permute(0, 2, 1).contiguous()  # B, H*W, L
