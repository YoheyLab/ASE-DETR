...
from ultralytics.nn.extra_modules.block import IFM, GLSA,Fusion,InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, \
    SimFusion_4in, AdvPoolFusion, PyramidPoolAgg, RepVGGBlock, Fused_Fourier_Conv_Mixer, GSConv, BasicBlock_Faster_Block, \
    RepConvN, SPDConv, CSPOmniKernel, DynamicAlignFusion, DWConv, EdgeEnhancer, MutilScaleEdgeInformationSelect
from ultralytics.nn.extra_modules.transformer import ASAEL
...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, ASAEL):
            ignored_layers.append(m.assa.attn)
            ignored_layers.append(m.assa.mlp)
            # ignored_layers.append(m.assa)
            ignored_layers.append(m.ffn.dwconv)
            ignored_layers.append(m.ffn.conv)
        # if isinstance(m, GLSA):
        #     ignored_layers.append(m)
    ...
    ignored_layers.append(model.model[12])
    ignored_layers.append(model.model[17])
